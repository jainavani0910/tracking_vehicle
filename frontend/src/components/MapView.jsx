...
const MapView = () => {
  ...
  useEffect(() => {
    if (selectedVehicleId) {
      ...
      vehicleAPI
        .getVehicleHistory(selectedVehicleId)
        .then((res) => {
        ...
        .catch((err) => {
          logger.error(
            `Failed to load history for ${selectedVehicleId}: ${err.message}`,
          );
        });
    } else {
      setSelectedVehicleHistory([]);
  ...
  const geojsonVehicles = useMemo(() => {
    const features = visibleVehicles.map((vehicle) => ({
      type: "Feature",
      geometry: {
        type: "Point",
      ...
      properties: {
        id: vehicle.id,
        name: vehicle.name?.split(" #")[1]
          ? `#${vehicle.name.split(" #")[1]}`
          : vehicle.id.substring(0, 5),
        speed: vehicle.speed,
    ...
    if (mapRef.current) {
      ...
      if (bounds) {
        ...
        for (let i = 0; i < vehicles.length; i++) {
          ...
          if (inLng && v.latitude >= south && v.latitude <= north) {
            visible.push(v);
          }
        }
      ...
      >
        ...
        <Source id="history-line" type="geojson" data={geojsonHistory}>
          <Layer
            ...
            paint={{
              "line-color": isDarkTheme ? "#e2e8f0" : "#0f172a",
              "line-width": 2.5,
              "line-opacity": 0.75,
              "line-dasharray": [2, 2],
            }}
        ...
        >
          ...
          <Layer
            ...
            paint={{
              ...
              "text-halo-color": isDarkTheme ? "#0f172a" : "#ffffff",
              "text-halo-width": 2,
            }}
          />
        </Source>
